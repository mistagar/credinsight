...
export async function fetchCustomers(): Promise<Customer[]> {
  ...
  const customers = data.$values || []; // Normalize each customer's nested arrays
  return customers.map((customer: RawApiCustomer) => {
    // Ensure transactions is always an array
    if (customer.transactions && '$values' in customer.transactions) {
    ...
    if (customer.loginActivities && '$values' in customer.loginActivities) {
...
export async function fetchCustomerStats(): Promise<CustomerStats> {
  ...
  const totalVerified = customers.filter((c) => c.isVerified).length;
  const activeUsers = customers.filter((c) => {
    // Ensure loginActivities is an array before using .some()
    return (
      Array.isArray(c.loginActivities) &&
      c.loginActivities.some((la) => {
...
export async function assessCustomerRisk(
  ...
  const response = await fetch(
    `${API_BASE_URL}/RiskAssessment/assess/${customerId}`,
    {
      method: 'POST',
    }
  );
...
export async function addCustomer(
  ...
  if (!response.ok) {
    ...
    throw new Error(
      `Error adding customer: ${JSON.stringify(errorData.errors || errorData)}`
    );
  }
